            ...
            @foreach ($community_posts as $community_post)
                <div class="media mt-5 profile-timeline-media">
                    <div class="align-self-start iconbox-45 overflow-hidden mr-3">
                        <img src="{{ asset('/images/admins/' . $community_post->admin->image) }}"
                    ...
                    <div class="media-body">
                        ...
                        <span class="float-right">
                            @php
                                $date = Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $community_post->created_at)->format('Y-m-d');
                            @endphp
                            {{ $date }}
                        </span>
                        <p><b style="color: black;">{{ $community_post->title }}</b><br>
                            {{ $community_post->text }}</p>
                        <div class="d-inline-block rounded overflow-hidden mt-4 mr-0 mr-lg-4">
                            {{-- src="{{asset('cpanel/assets/img/products/pa3.jpg')}}" --}}
                            <img @if (!is_null($community_post->image)) src="{{ asset('/images/posts/' . $community_post->image) }}"
                            @else
